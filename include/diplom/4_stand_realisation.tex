...
\midrule
\texttt{bthci} & Отображает все HCI-пакеты (база для любого анализа). \\
\texttt{bthci\_cmd} & Только HCI-команды (от Host к Controller). \\
\texttt{bthci\_evt} & Только HCI-события (от Controller к Host). \\
\texttt{btl2cap} & Пакеты протокола L2CAP. \\
\texttt{btrfcomm} & Трафик профиля RFCOMM (Classic BT). \\
\texttt{btatt} & Протокол ATT (основа BLE). \\
\texttt{btgatt} & Профиль GATT (структура сервисов BLE). \\
\texttt{bthci\_evt.code == 0x03} & Событие завершения подключения (Connection Complete). \\
\texttt{bthci\_evt.code == 0x2f} & Расширенные результаты поиска (Extended Inquiry Result). \\
\bottomrule
\end{tabularx}
\end{table}
